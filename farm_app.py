...
with tabs[1]:
    ...
    with col1:
        ...
        with st.form("livestock_form"):
            ...
            with col_b:
                ...
                manager = st.selectbox("Expense Managed By *", [""] + managers)
    ...
    if not livestock_data.empty:
        ...
        with col_s4:
            avg_cost_per_animal = livestock_data[livestock_data['transaction_type'] == 'expense']['amount'].sum() / \
                                 livestock_data['quantity'].sum() if livestock_data['quantity'].sum() > 0 else 0
...
with tabs[2]:
    ...
    with col1:
        ...
        with st.form("crop_form"):
            ...
            with col_a:
                date = st.date_input("Crop Date *", value=datetime.now().date())
            ...
            with col_b:
                amount = st.number_input("Crop Amount (PKR) *", min_value=0.0, value=0.0)
                managers_df = pd.read_sql("SELECT name FROM managers", conn)
                managers = managers_df['name'].tolist() if not managers_df.empty else []
                manager = st.selectbox("Crop Manager *", [""] + managers)
                transaction_type = st.selectbox("Crop Transaction Type *", 
                                              ["expense", "income"])
                remarks = st.text_area("Crop Remarks")
        ...
        with col_f1:
            filter_crop = st.selectbox("Filter by Crop Type", 
                                     ["All"] + ["Wheat", "Rice", "Cotton", "Kheera (Cucumber)", 
                                               "Corn", "Vegetables", "Others"])
        with col_f2:
            crop_date = st.date_input("Filter Date", value=None)
...
with tabs[3]:
    ...
    with st.form("water_supply_form"):
        ...
        with col_w1:
            ...
            date = st.date_input("Supply Date *", value=datetime.now().date())
        
        with col_w2:
            # Manual time input
            manual_hours = st.number_input("Manual Hours (override)", min_value=0.0, value=hours, step=0.1)
            ...
            remarks = st.text_area("Water Supply Remarks")
...
with tabs[4]:
    ...
    with col1:
        ...
        with st.form("expense_form"):
            ...
            with col_a:
                date = st.date_input("Expense Date *", value=datetime.now().date())
            ...
            with col_b:
                ...
                remarks = st.text_area("Expense Remarks")
    ...
    if managers_expense:
        selected_manager = st.selectbox("Select Manager", managers_expense)
        ...
        if selected_manager:
            ...
            with col_m1:
                manager_date_from = st.date_input("From Date", value=None, key="manager_from")
            with col_m2:
                manager_date_to = st.date_input("To Date", value=None, key="manager_to")
...
with tabs[5]:
    ...
    with col1:
        ...
        with st.form("income_form"):
            ...
            with col_a:
                date = st.date_input("Income Date *", value=datetime.now().date())
                # Updated income sources as requested
                source = st.selectbox("Income Source *", 
                                    ["", "Goats", "Beef", "Cows", "Crop Sale", 
                                     "Water Supply", "Others"])
                amount = st.number_input("Income Amount (PKR) *", min_value=0.0, value=0.0)
            ...
            with col_b:
                ...
                receipt_no = st.text_input("Receipt No")
                remarks = st.text_area("Income Remarks")
...
with st.sidebar:
    ...
    with st.form("manager_form"):
        ...
        manager_phone = st.text_input("Phone Number")
        manager_designation = st.text_input("Designation")
    ...
    with st.form("farmer_form"):
        farmer_name = st.text_input("Farmer Name")
        farmer_phone = st.text_input("Farmer Phone")
    ...
    with col_b2:
        uploaded_file = st.file_uploader("Restore", type=['db'])
