...
with tabs[3]:
    ...
    if farmers:
        ...
        if selected_farmer:
            ...
            with col_sum1:
                st.metric("Total Bill", f"PKR {total_bill:,.0f}", key="farmer_total_bill")
            with col_sum2:
                st.metric("Total Paid", f"PKR {total_paid:,.0f}", key="farmer_total_paid")
            with col_sum3:
...
with tabs[4]:
    ...
    with col2:
        ...
        if not expense_data.empty:
            ...
            col_m1, col_m2, col_m3 = st.columns(3)
            with col_m1:
                st.metric("Today's Expenses", f"PKR {today_expenses:,.0f}", key="today_expenses")
            with col_m2:
                st.metric("This Month", f"PKR {month_expenses:,.0f}", key="month_expenses")
            with col_m3:
                st.metric("Total Expenses", f"PKR {total_expenses:,.0f}", key="total_expenses")
    ...
    if managers_expense:
        ...
        if selected_manager:
            ...
            st.metric(f"Total Expenses by {selected_manager}", 
                     ...
                     key=f"manager_total_{selected_manager}")
...
with tabs[5]:
    ...
    with col2:
        ...
        with col_i1:
            st.metric("Livestock Income", f"PKR {livestock_income:,.0f}", key="livestock_income_metric")
        with col_i2:
            st.metric("Crop Income", f"PKR {crop_income:,.0f}", key="crop_income_metric")
        with col_i3:
            st.metric("Water Supply Income", f"PKR {water_income:,.0f}", key="water_income_metric")
        with col_i4:
            st.metric("Total Income", f"PKR {total_income:,.0f}", key="total_income_metric")
...
with tabs[6]:
    ...
    if st.button("📈 Generate Report", type="primary", key="generate_report"):
        ...
        if report_type == "Summary Report":
            ...
            col_s1, col_s2, col_s3, col_s4 = st.columns(4)
            with col_s1:
                st.metric("Total Income", f"PKR {grand_total_income:,.0f}", key="report_total_income")
            with col_s2:
                st.metric("Total Expenses", f"PKR {grand_total_expense:,.0f}", key="report_total_expenses")
            with col_s3:
                st.metric("Net Balance", f"PKR {net_balance:,.0f}", key="report_net_balance")
            with col_s4:
                st.metric("Profit Margin", f"{profit_margin:.1f}%", key="report_profit_margin")
            ...
            if not water_df.empty:
                ...
                with col_w1:
                    st.metric("Total Bill", f"PKR {total_bill:,.0f}", key="water_total_bill")
                with col_w2:
                    st.metric("Total Collected", f"PKR {total_paid:,.0f}", key="water_total_paid")
                with col_w3:
                    st.metric("Collection Rate", f"{collection_rate:.1f}%", key="water_collection_rate")
            ...
            with col_bs1:
                ...
                st.metric("Total Assets", f"PKR {total_assets:,.0f}", key="balance_total_assets")
            ...
            with col_bs2:
                ...
                st.metric("Total Liabilities", f"PKR {total_liabilities:,.0f}", key="balance_total_liabilities")
            
            st.markdown("---")
            col_eq1, col_eq2, col_eq3 = st.columns(3)
            with col_eq2:
                st.metric("Owner's Equity", f"PKR {equity:,.0f}", 
                         delta=f"{(equity/total_assets*100 if total_assets > 0 else 0):.1f}%" if equity >= 0 else None,
                         key="balance_equity")
