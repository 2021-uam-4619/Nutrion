...
class PDF(FPDF):
    def __init__(self, report_title="Report", date_range_str="", logo_path="logo.png"):
        super().__init__()
...
uploaded_file = st.file_uploader("Upload your CSV file (Expense or Salary Data)", type=['csv'])

if uploaded_file:
    df = pd.read_csv(uploaded_file)
    st.dataframe(df)

    report_title = st.text_input("Enter Report Title", "Expense or Salary Report")
    date_range = st.text_input("Enter Date Range", "01 Nov 2025 - 30 Nov 2025")

    if st.button("Generate PDF"):
        pdf = PDF(report_title=report_title, date_range_str=date_range)
        pdf.alias_nb_pages()
        pdf.add_page()
