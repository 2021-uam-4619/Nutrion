...
def generate_employee_template():
    ...
    template_data = {
        ...
        'account_no': ['123456789', '987654321'],
        'join_date': ['2024-01-01', '2024-01-15'],
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        worksheet.write_comment('A1', 'Required fields: name, designation, salary')
    output.seek(0)
    return output
...
def generate_expense_template():
    ...
    template_data = {
        'description': ['Office Supplies', 'Client Meeting'],
        'amount': [5000, 8000],
        'expense_date': ['2024-01-10', '2024-01-15'],
        'category_id': [1, 2],
        'employee_id': [1, 2]
    }
    df = pd.DataFrame(template_data)
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        worksheet.write_comment('A1', 'Required fields: description, amount, expense_date')
    output.seek(0)
    return output

def generate_ledger_template():
    """Generate ledger import template"""
    template_data = {
        'employee_id': [1, 2],
        'entry_date': ['2024-01-20', '2024-01-25'],
        'description': ['Travel Advance', 'Bonus Payment'],
        'debit': [5000, 0],
        'credit': [0, 10000]
    }
    df = pd.DataFrame(template_data)
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        worksheet.write_comment('A1', 'Required fields: employee_id, entry_date, description')
    output.seek(0)
    return output
...
def page_employee_management():
    ...
    with tab2:
        ...
        try:
            ...
            st.dataframe(
                employees_df[['id', 'name', 'designation', 'salary', 'bank', 'status']].style.format({
                    'salary': 'Rs. {:,.2f}'
                }),
                use_container_width=True
            )
...
def page_expense_management():
    ...
    st.info("This page is for company expenses that are NOT linked to specific employees.")
    
    tab1, tab2 = st.tabs(["➕ Add Company Expense", "📋 Expense Records"])
...
def page_import_export():
    ...
    with tab1:
        ...
        uploaded_file = st.file_uploader(
            f"Upload {import_type} File (Excel)",
            type=['xlsx'],
    ...
    with tab3:
        ...
        with col1:
            st.write("**Employees Template**")
            st.write("Use this template to import employee data")
        ...
        with col2:
            st.write("**Expenses Template**")
            st.write("Use this template to import company expenses")
            template_data = generate_expense_template()
            st.download_button(
        ...
        with col3:
            st.write("**Ledger Template**")
            st.write("Use this template to import employee ledger entries")
            template_data = generate_ledger_template()
            st.download_button(
        ...
        if employees_df.empty:
            ...
            with col1:
                report_type = st.selectbox(
                    "Report Type",
                    ["Salary Sheet", "Employee Directory"]
                )
            with col2:
                ...
                try:
                    ...
                    if not df.empty:
                        ...
                        pdf_bytes = generate_pdf_report(
                            df, 
                            title,
                            date_range=(first_day, last_day) if report_type == "Salary Sheet" else None,
                            orientation='L',
...
def page_dashboard():
    ...
    **📋 Navigation Guide:**
    ...
    - **📤 Import/Export**: Bulk import/export data with templates
    """)
