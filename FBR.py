...
def search_invoices(search_term):
    ...
    for row in c.fetchall():
        invoices.append({
            ...
            'items': json.loads(row[7]), 'subtotal': row[8], 'gst_total': row[9],
    ...
    with tab1:
        ...
        for i, item in enumerate(st.session_state.invoice_items_without_gst):
            with st.container():
                ...
                # Remove button
                if st.button("Remove", key=f"remove_without_{i}") and len(st.session_state.invoice_items_without_gst) > 1:
    ...
    with tab2:
        ...
        for i, item in enumerate(st.session_state.invoice_items_with_gst):
            with st.container():
                ...
                # Remove button
                if st.button("Remove", key=f"remove_with_{i}") and len(st.session_state.invoice_items_with_gst) > 1:
            ...
            if invoices:
                ...
                for invoice in invoices:
                    with st.expander(f"Invoice {invoice['invoice_number']} - {invoice['party_name']}"):
                        ...
                        with col1:
                            if st.button(f"Load", key=f"load_{invoice['id']}"):
                        ...
                        with col3:
                            if st.button(f"Delete", key=f"delete_{invoice['id']}"):
...
def render_bulk_download():
    ...
    with col1:
        if st.button("Download Tax Invoices"):
            invoices = get_invoices_by_date_range(start_date.isoformat(), end_date.isoformat(), 'tax')
            if invoices:
                # Create a combined PDF (simplified - in reality you'd want to create a proper combined PDF)
                st.info(f"Found {len(invoices)} tax invoices. Download functionality would be implemented here.")
                # Actual implementation would create a zip file or combined PDF
            else:
                st.warning("No tax invoices found in the selected date range.")
    
    with col2:
        if st.button("Download Non-Tax Invoices"):
            invoices = get_invoices_by_date_range(start_date.isoformat(), end_date.isoformat(), 'non-tax')
            if invoices:
                st.info(f"Found {len(invoices)} non-tax invoices. Download functionality would be implemented here.")
            else:
